...
if os.path.isdir(run_directory):
    ...
    def align_with_bwa(input_file, output_file, extras):
        ...
        reference = "/data/humanGenome/hs37d5"
        ...
        os.system("bwa mem -M -t 16 %s -R '%s' %s %s > %s 2>%s" % (reference, read_group, fastq_1, fastq_2, sam_file, log_file))
    ...
    def index_bam(input_file, output_files, extras):
        ...
        os.system("java -Xmx2g -Djava.io.tmpdir=/data/tmp -jar %s INPUT=%s OUTPUT=%s SORT_ORDER=coordinate TMP_DIR=/data/tmp &>%s" % (picard_SortSam, bam, sorted_bam, picardSort_log))
    ...
    def dedup_bam(input_file, output_files):
        ...
        os.system('java -Xmx8g -Djava.io.tmpdir=/data/tmp -jar %s INPUT=%s OUTPUT=%s METRICS_FILE=%s TMP_DIR=/data/tmp &>%s' % (mark_duplicates, input_file[0], dedup_bam, picard_metrics, picard_log))
    ...
    def call_variants(inputs, output_file, log_file):
        ...
        bed = fh_bed
        os.system('java -jar %s -T HaplotypeCaller --annotation StrandAlleleCountsBySample -R %s -I %s -L %s -o %s &>%s' % (gatk_jar, human_decoy, input_bam, bed, output_file, log_file))
